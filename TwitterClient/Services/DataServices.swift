...
class DataServices {
    ...
    func fetchFollowers(completion: @escaping (Bool,[RealmFollowerr]?) -> ()) {//
        let screenName = userDefaults.value(forKey: "SCREEN_NAME")
        
        let url = "https://api.twitter.com/1.1/followers/list.json?screen_name=\(screenName!)&count=5&cursor=-1"
    ...
    func fetchAnotherFollowers(completion: @escaping (Bool,[RealmFollowerr]?) -> ()) {
        let screenName = userDefaults.value(forKey: "SCREEN_NAME")
        
        let url = "https://api.twitter.com/1.1/followers/list.json?screen_name=\(screenName!)&count=5&cursor=\(FOLLOWERS_next_cursor_str)"
        
        print("urlFollowers2: \(url)")
    ...
    func fetchAnotherUserTweets(screenName: String,max_id: Int, sinceId: Int , completion: @escaping (Bool,[RealmTweet]?) -> ()) {
        
        //Get your user timeline
        let url = "https://api.twitter.com/1.1/statuses/user_timeline.json?screen_name=\(screenName)&since_id=\(sinceId)"//&max_id=\(max_id)
